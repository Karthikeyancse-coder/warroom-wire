...
export async function POST(req: Request) {
  try {
    ...
    if (expectedSecret && authHeader !== `Bearer ${expectedSecret}`) {
      return NextResponse.json({ error: "Unauthorized: Invalid ingest token" }, { status: 401 });
    }
    ...
    const article: Omit<Article, "id" | "created_at" | "source_id"> & { content_hash?: string; verified?: boolean } = {
      ...
      author:       author ? String(author).slice(0, 100) : "nostr:user",
    ...
    // Upsert into Supabase (ON CONFLICT content_hash DO NOTHING via ignoreDuplicates)
    ...
    try {
      ...
      await supabase
        ...
        .eq("name", "nostr");
    } catch { /* supabase offline — in-memory fallback already done */ }
...
export async function GET() {
  return NextResponse.json({ message: "Nostr ingestion endpoint ready. Use POST with Bearer token." });
}